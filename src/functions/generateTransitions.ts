...
function createTransitionValue(property: string, durationMs: number, easing: string) {
  if (durationMs < 5 && process.env.NODE_ENV === 'development')
    console.warn('`createTransitionValue` called with duration < 5ms. Are you sure this was passed in milliseconds?');
